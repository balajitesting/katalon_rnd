...
int countBeforeRelease

while(rs1.next()){
countBeforeRelease = rs1.getInt("count");
}
System.out.println(countBeforeRelease);
...
//String queryAfterRelease = "select COUNT(*) as count from U_GHFAX U, S_REQUEST S WHERE U.REQUESTID = 'A80196' AND S.S_REQUESTID = U.REQUESTID ORDER BY S.MODDT DESC"
ResultSet rs2 = db.executeQuery(query);

int countAfterRelease

while(rs2.next()){
countAfterRelease = rs2.getInt("count");
}
