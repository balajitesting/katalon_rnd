...
public class LimsDBDataReset {
	 ...
	 */
	@Keyword
	def deleteReagentLot(String regentLotId) {

		LimsOracleDBService db = new LimsOracleDBService();
		db.connectDB(GlobalVariable.oracleDBurl, GlobalVariable.oracleDBuser, GlobalVariable.oracleDBpwd);

		String query1 = "DELETE FROM REAGENTLOT WHERE REAGENTLOTID = '"+regentLotId+"'";
		db.execute(query1);

		String query2 = "DELETE FROM A_REAGENTLOT WHERE REAGENTLOTID = '"+regentLotId+"'";
		db.execute(query2);

		String query3 = "DELETE FROM S_SDICERTIFICATION WHERE RESOURCEKEYID1 = '"+regentLotId+"'";
		db.execute(query3);
	}
}
